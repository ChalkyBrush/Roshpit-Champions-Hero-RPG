...
CustomAttributes.WARLORD_Q4_STATS = 750
...
function CDOTA_BaseNPC:RoshpitSetPhysicalArmorBaseValue(base_armor)
	...
	if not self:IsRealHero() then
		...
		self:SetModifierStackCount("modifier_physical_armor", Events.GameMaster, base_armor)
		Events.GameMasterAbility:ApplyDataDrivenModifier(Events.GameMaster, self, "modifier_armor_calculate", {})
		self:CalculateTotalArmor()
	end
end
...
function CDOTA_BaseNPC:CalculateTotalArmor()
	...
	if armor_bonus > 0 then
		...
		self:SetModifierStackCount("modifier_physical_armor_bonus", Events.GameMaster, armor_bonus)
	elseif armor_bonus < 0 then
		self:RemoveModifierByName("modifier_physical_armor_bonus")
		Events.GameMasterAbility:ApplyDataDrivenModifier(Events.GameMaster, self, "modifier_physical_armor_bonus_negative", {})
		self:SetModifierStackCount("modifier_physical_armor_bonus_negative", Events.GameMaster, armor_bonus*-1)
	else
		self:RemoveModifierByName("modifier_physical_armor_bonus")
		self:RemoveModifierByName("modifier_physical_armor_bonus_negative")
	end
	if base_armor > 0 then
		Events.GameMasterAbility:ApplyDataDrivenModifier(Events.GameMaster, self, "modifier_physical_armor", {})
		self:SetModifierStackCount("modifier_physical_armor", Events.GameMaster, base_armor)
	else
		self:RemoveModifierByName("modifier_physical_armor")
	end
	return true
end
