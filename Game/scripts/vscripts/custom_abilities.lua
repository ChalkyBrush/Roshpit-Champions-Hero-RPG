...
function CustomAbilities:ChernobogDemonHunter(victim, damage)
	...
	--print("THRESHOLD!!")
	--print(threshold)
	if victim:HasModifier("modifier_chernobog_glyph_7_1") then
		threshold = threshold - CHERNOBOG_GLYPH_7_1_W_THRESHOLD_REDUC
	end
	...
	elseif msg.unit_name == "gem_forger" then
		...
		if player then
			...
			if distance <= distance_cap then
				...
				CustomGameEventManager:Send_ServerToPlayer(player, "open_gemforger", {player=playerID, gem_reward = gem_reward, go_home = Gems.GemForger.go_home} )
				CustomGameEventManager:Send_ServerToPlayer(player, "select_hero", {} )
			else
				Notifications:Top(playerID, {text="Too Far", duration=4, style={color="#FFDDAA"}, continue=true})
				CustomGameEventManager:Send_ServerToPlayer(player, "grey_dialogue", {player=playerID} )
...
function CDOTA_BaseNPC_Hero:ReduceAllCurrentCooldowns(time)
	local ability_slots = {{DOTA_Q_SLOT, GLOBAL_Q_MIN_CD}, 
			       {DOTA_W_SLOT, GLOBAL_W_MIN_CD}, 
			       {DOTA_E_SLOT, GLOBAL_E_MIN_CD}, 
			       {DOTA_R_SLOT, GLOBAL_R_MIN_CD}}
	for i = 1, #ability_slots, 1 do
		local ability = self:GetAbilityByIndex(ability_slots[i][1])
		
		if ability then
			local remainingCD = ability:GetCooldownTimeRemaining()
			if remainingCD > 0 then
				local newCD = math.max(ability_slots[i][2], remainingCD - time)
				if newCD > 0 then
					ability:EndCooldown()
