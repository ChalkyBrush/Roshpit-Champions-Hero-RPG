...
function neutral_glyph_4_3_think(event)	
	for k,item in pairs(GLOBAL_ITEM_TABLE) do        
        if item and not item:IsNull() then
            if item.expiryTime and item.slot and item.property1name then
                if item.slot == "consumable" and item.property1name == "heal" and item.expiryTime - Time() > 1 then --recently dropped potions
                    ...
                    if distanceToPotion < 400 then--only nearby potions
	                    ...
	                    if item.property2name then--eating only tastie potions
