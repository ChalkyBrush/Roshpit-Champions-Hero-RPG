...
function itemClass:RollProperty1(item_level)
    ...
    self:SetSpecialValue("twisted_yellow_mask_of_ahnqhir", TWISTED_MASK_OF_AHNQHIR_YELLOW_COLOR)
end
...
function modifierClass:OnIntervalThink()
	...
	if pointAbility then
		if pointAbility.ahnqhirPoint then
			...
			pointAbility:SetOverrideCastPoint(TWISTED_MASK_OF_AHNQHIR_YELLOW_CAST_POINT)
		end
	end
end
...
function modifierClass:GetRoshpitWFlatCdModifier()
    return - TWISTED_MASK_OF_AHNQHIR_YELLOW_CD_RED
end
function modifierClass:GetRoshpitTooltipW()
    return { 
        itemIndex = self:GetAbility():GetEntityIndex(), 
        color = TWISTED_MASK_OF_AHNQHIR_YELLOW_COLOR, 
        immortal = true, 
