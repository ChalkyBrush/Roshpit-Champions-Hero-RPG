...
function Spawn( entityKeyValues )
    ...
    behaviorSystem = AICore:CreateBehaviorSystem( { BehaviorNone, BehaviorMysticFlare} ) 
end
...
function BehaviorMysticFlare:Evaluate()
	...
	self.flareAbility = thisEntity:FindAbilityByName( "furion_mystic_flare" )
	
	if self.flareAbility and self.flareAbility:IsFullyCastable() then
		self.target = AICore:RandomEnemyHeroInRange( thisEntity, self.flareAbility:GetCastRange() )
		if self.target then
			desire = 4
		end
	end
...
function BehaviorMysticFlare:Begin()
	...
	if self.target then
		...
		self.order =
		{
			...
			AbilityIndex = self.flareAbility:entindex(),
