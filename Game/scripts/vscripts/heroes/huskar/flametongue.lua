...
function flametongue_attack_land(event)
	...
	if target:GetPhysicalArmorValue() < 0 then
		damage = damage + (event.negative_armor_amp/100)*math.abs(target:GetPhysicalArmorValue())*damage
	end
	damage = damage*mult
...
function a_a_damage(event)
	...
	if target:GetPhysicalArmorValue() < 0 then
		burnDamage = burnDamage + (event.negative_armor_amp/100)*math.abs(target:GetPhysicalArmorValue())*burnDamage
	end
