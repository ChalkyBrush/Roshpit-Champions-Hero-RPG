...
function zonik_passive_think(event)
	...
	if r_2_level > 0 then
		if caster:IsRooted() or caster:IsStunned() then
			...
			caster:SetModifierStackCount("modifier_speedball_b_d_regen", caster, r_2_level * ZHONIK_R2_HEALTH_REGEN)
			caster:SetModifierStackCount("modifier_speedball_b_d_mana_regen", caster, r_2_level * ZHONIK_R2_MANA_REGEN)
		end
	else
		caster:RemoveModifierByName("modifier_speedball_b_d_regen")
		caster:RemoveModifierByName("modifier_speedball_b_d_mana_regen")
...
function speedball_explode(caster, ability, damage, stun_duration)
	...
	if #enemies > 0 then
		for _, enemy in pairs(enemies) do
			Filters:TakeArgumentsAndApplyDamage(enemy, caster, damage, DAMAGE_TYPE_MAGICAL, BASE_ABILITY_R, RPC_ELEMENT_TIME, RPC_ELEMENT_COSMOS)
