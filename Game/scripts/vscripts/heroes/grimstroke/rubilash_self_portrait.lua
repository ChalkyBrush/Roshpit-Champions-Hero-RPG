...
function rubilash_dark_portrait_channel_start(event)
	...
	ability.illusion = rubilash_illusion(caster, ability)
...
function rubilash_illusion(caster, ability)
    local modifierKeys = {}
    modifierKeys.outgoing_damage = 0
    modifierKeys.incoming_damage = 1 - (30/100)
    modifierKeys.duration = 20

    local illusions = CreateIllusions( caster, caster, modifierKeys, 1, 20, false, false)
...
function rubilash_self_portrait_success(event)
	...
	if ability.illusion and IsValidEntity(ability.illusion) then
		...
		CustomAbilities:QuickAttachParticle("particles/roshpit/rubilash/self_portrait_buff_"..ability.illusion.color..".vpcf", ability.illusion, 20)
