local function cast(caster, target, ability, damage)
    ...
    ParticleManager:SetParticleControl(particle1, 1, Vector(SORCERESS_Q3_RADIUS, 2, 1000))
    ParticleManager:SetParticleControl(particle1, 3, Vector(SORCERESS_Q3_RADIUS, 550, 550))
    Timers:CreateTimer(3, function()
        ParticleManager:DestroyParticle(particle1, false)
    end)
    local freezeDuration = SORCERESS_Q3_BASE_DURATION + runesCount * SORCERESS_Q3_ADD_DURATION
    local damage = damage * SORCERESS_Q3_AMPLIFY_PERCENT / 100 * runesCount
    local radius = SORCERESS_Q3_RADIUS
    ...
    if #enemies > 0 then
        for _, enemy in pairs(enemies) do
            if Immune.shouldApplyImmune(caster, enemy, 'modifier_ice_lance', SORCERESS_Q3_TIME_BEFORE_IMMUNE) then
                Immune.applyImmune(caster, enemy, ability, 'modifier_ice_lance', SORCERESS_Q3_IMMUNE_DURATION)
