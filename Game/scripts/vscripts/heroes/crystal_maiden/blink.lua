  ...
  if #enemies > 0 then
    for _, enemy in pairs(enemies) do
		...
		if not enemy.dummy then
			enemy:MoveToTargetToAttack(attacker)
		end
    end
  end
...
function elemental_extra_attack_strike(event)
  ...
  if #enemies > 0 then
    for _, enemy in pairs(enemies) do
      Filters:TakeArgumentsAndApplyDamage(enemy, attacker, damage, DAMAGE_TYPE_MAGICAL, BASE_ABILITY_E, RPC_ELEMENT_ICE, RPC_ELEMENT_NONE)
      ability:ApplyDataDrivenModifier(attacker, enemy, "modifier_elemental_slow", {duration = slowDuration})
    end
  end
end
