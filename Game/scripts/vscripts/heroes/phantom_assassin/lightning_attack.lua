        ...
        if not caster:IsIllusion() then
            ...
            if caster.hero.q_3_level then
                if caster.hero.q_3_level > 0 then
                    ...
                    local luck = RandomInt(1, 10)
                    if luck <= 3 then
                        local q3damage = OverflowProtectedGetAverageTrueAttackDamage(caster.hero)*0.5*caster.hero.q_3_level
                        Filters:TakeArgumentsAndApplyDamage(unit, caster.hero, q3damage, DAMAGE_TYPE_PHYSICAL, 1, RPC_ELEMENT_LIGHTNING, RPC_ELEMENT_NONE)
                    end
                end
            end
        end
