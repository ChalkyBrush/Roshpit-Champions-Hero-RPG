...
function cast(event)
    ...
    local vipers_count = 1

    local r4_level = Runes:GetTotalRuneLevelGeneric(caster, 4, 3)
    if r4_level > 0 then
        vipers_count = vipers_count + Runes:Procs(r4_level, constants.R4_PROC_CHANCE, 1)
    end
    ...
    EmitSoundOn("Hero_Morphling.AdaptiveStrike.Cast", viper)

    for i = 1,vipers_count do
