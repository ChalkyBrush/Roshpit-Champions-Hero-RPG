...
function channel_complete(event)
	...
	for i = 0, castLoops, 1 do
		Timers:CreateTimer(i*2, function()
			...
			damage = damage * (1 + HYDROXIS_R1_BAD_PCT_PER_ARMOR * r_1_level * caster:RoshpitGetPhysicalArmorValue())
			if #enemies > 0 then
...
function tsunami_impact(event)
	...
	local ability = event.ability
	local caster = event.caster
	local damage = 85000*ability.r_2_level * (1 + HYDROXIS_R1_BAD_PCT_PER_ARMOR * ability.r_1_level * caster:RoshpitGetPhysicalArmorValue()) * HYDROXIS_R1_RUNE_MULT
...
function poseidon_wrath_attack_hit(event)
	...
	local target = event.target
	local ability = event.ability
	local damage = ability.r_3_level*0.35*OverflowProtectedGetAverageTrueAttackDamage(caster) * (1 + HYDROXIS_R1_BAD_PCT_PER_ARMOR * ability.r_1_level * caster:RoshpitGetPhysicalArmorValue()) * HYDROXIS_R1_RUNE_MULT
	if caster:HasModifier("modifier_hydroxis_immortal_weapon_1") then
		damage = damage*2
	end
	Filters:TakeArgumentsAndApplyDamage(target, caster, damage, DAMAGE_TYPE_PURE, 0, RPC_ELEMENT_WATER, RPC_ELEMENT_NONE)
end