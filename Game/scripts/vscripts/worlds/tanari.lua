...
function Tanari:Debug()
    ...
    -- RPCItems:RollAndDropUniqueItem(unit, "item_rpc_alaranas_ice_boot")
    ...
    local helm = RPCItems:RollAndDropImmortalByLevel(MAIN_HERO_TABLE[1]:GetAbsOrigin(), 60, "item_rpc_alaranas_ice_boot")

    helm = Gems:AddSocket(helm)
    helm = Gems:AddSocket(helm)

    local helm = RPCItems:RollAndDropImmortalByLevel(MAIN_HERO_TABLE[1]:GetAbsOrigin(), 1, "item_rpc_alaranas_ice_boot")
