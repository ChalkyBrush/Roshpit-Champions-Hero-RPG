...
function Tanari:Debug()
    ...
    local helm = RPCItems:RollAndDropImmortalByLevel(MAIN_HERO_TABLE[1]:GetAbsOrigin(), 60, "item_rpc_direwolf_bulwark")

    helm = Gems:AddSocket(helm)
    helm = Gems:AddSocket(helm)

    local helm = RPCItems:RollAndDropImmortalByLevel(MAIN_HERO_TABLE[1]:GetAbsOrigin(), 1, "item_rpc_direwolf_bulwark")
