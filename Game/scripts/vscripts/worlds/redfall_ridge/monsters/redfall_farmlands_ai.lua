function PurifiedVermillionBundleOnSpellStart(event)
	local caster = event.caster
	local ability = event.ability
	local casterOrigin = caster:GetAbsOrigin()
	local shrinePosition = Vector(-61, -7275)
	local teleportPosition = Vector(-15808, -15424)
	local distance = WallPhysics:GetDistance2d(casterOrigin, shrinePosition)
	if not Redfall.FarmlandsPortalActive then
		if distance < 120 then
			Redfall.FarmlandsPortalActive = true
			Dungeons.respawnPoint = shrinePosition
			local particlePosition = shrinePosition + Vector(0, 0, 156 + Redfall.ZFLOAT)
			...
			local bridge = Entities:FindAllByNameWithin("RedfallForest_RedfallFarmlands_Bridge", Vector(1728, -7282, 100 + Redfall.ZFLOAT), 3000)
			...
			for j = 1, steps, 1 do
				for i = 1, #bridge, 1 do
					Timers:CreateTimer(j * 0.03, function()
						bridge[i]:SetAbsOrigin(bridge[i]:GetAbsOrigin() + Vector(0, 0, height / steps))
					end)
				end
			end
			...
			Dungeons.respawnPoint = Vector(-61, -7275)
			UTIL_Remove(ability)
			...
			Timers:CreateTimer(1.0, function()
				...
				Beacons:CreateActiveParticle("particles/portals/green_portal.vpcf", teleportPosition + Vector(0, 0, 120 + Redfall.ZFLOAT), Events.GameMaster, 0, Vector(0.45, 0.45, 0.45))
			end)
			Redfall:SpawnFarmlandsPt1()
		else
			MinimapEvent(caster:GetTeamNumber(), caster, shrinePosition.x, shrinePosition.y, DOTA_MINIMAP_EVENT_BASE_UNDER_ATTACK, 4)
		end
	end
end
...
function use_shipyard_key(event)
	...
	if distance < 130 then
		...
		MinimapEvent(DOTA_TEAM_GOODGUYS, caster, -13661, -8425, DOTA_MINIMAP_EVENT_HINT_LOCATION, 4)
		EmitSoundOnClient("General.Cancel", caster:GetPlayerOwner())
	end
