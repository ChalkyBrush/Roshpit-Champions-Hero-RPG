...
function ancient_tree_main_think(event)
	...
	local naturesGraspAbility = caster:FindAbilityByName("ancient_tree_natures_grasp")
	if naturesGraspAbility:IsFullyCastable() then
		...
		if #enemies > 0 then
			local newOrder = {
				...
				AbilityIndex = naturesGraspAbility:entindex(),
			}
...
function AncientTreeSummonPhotosynthesisOnCreated(event)
	--particles/units/heroes/hero_pugna/pugna_life_drain.vpcf
end

function AncientTreeSummonPhotosynthesisOnIntervalThink(event)
	local summon = event.target
	local boss = summon.origCaster
	if not summon.particle then
		summon.particle = ParticleManager:CreateParticle("particles/units/heroes/hero_pugna/pugna_life_drain_beam_2b_give.vpcf", PATTACH_POINT_FOLLOW, summon)
		ParticleManager:SetParticleControlEnt(summon.particle, 0, summon, PATTACH_POINT_FOLLOW, "attach_hitloc", summon:GetAbsOrigin(), true)
		ParticleManager:SetParticleControlEnt(summon.particle, 1, boss, PATTACH_POINT_FOLLOW, "attach_hitloc", boss:GetAbsOrigin(), true)
	end
	Filters:ApplyHeal(summon, summon, summon:GetMaxHealth() * event.heal_percent_self / 100 * 0.03, true)
	Filters:ApplyHeal(summon, boss, boss:GetMaxHealth() * event.heal_percent_boss / 100 * 0.03, true)
end

function AncientTreeSummonPhotosynthesisOnDestroy(event)
	ParticleManager:DestroyParticle(event.target.particle, false)
	event.target.particle = false
end
