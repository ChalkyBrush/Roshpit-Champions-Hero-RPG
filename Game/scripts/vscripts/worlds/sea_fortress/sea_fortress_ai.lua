...
function lady_summoner_think(event)
	...
	if caster.state then
		...
		elseif caster.state == 3 then
			...
			local enemies = FindUnitsInRadius( caster:GetTeamNumber(), caster:GetAbsOrigin(), nil, 450, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, 0, FIND_ANY_ORDER, false )
			if #enemies > 0 then
