...
function AddSpecialDescriptionToTooltip(tooltip, itemProperty1, itemProperty2, itemProperty3, itemProperty4, rarityFactor)
{
...
function OnDragDrop( panelId, draggedPanel )
{
	...
	if ((m_ItemSlot <= 6) && (draggedPanel.m_type == "inventory" || draggedPanel.m_type == "trade") && ($.GetContextPanel().lockCheckPanel.locked == 0) && (dupeCheck)){
		...
		GameEvents.SendCustomGameEventToServer( "item_placed_in_trade", {playerID: playerID, itemIndex: draggedItem, slot: m_ItemSlot});
		// m_Item = draggedItem
		// SetItem( -1, m_Item )
...
function OnDragStart( panelId, dragCallbacks )
{
	...
	displayPanel.m_DragCompletedTrade = false
...
function OnDragEnd( panelId, draggedPanel )
{
	...
	$.Msg("gone?")
	if ($.GetContextPanel().lockCheckPanel.locked == 0){
		if ( !draggedPanel.m_DragCompletedTrade ){
			$.Msg("RETURN ITEM")
			GameEvents.SendCustomGameEventToServer( "item_removed_from_trade", {playerID: playerID, itemIndex: draggedItem, slot: m_ItemSlot, bReturnItem: 1});
		}else{
			$.Msg("DONT RETURN ITEM")
			GameEvents.SendCustomGameEventToServer( "item_removed_from_trade", {playerID: playerID, itemIndex: draggedItem, slot: m_ItemSlot, bReturnItem: 0});
		}
	}
	return true;
}
