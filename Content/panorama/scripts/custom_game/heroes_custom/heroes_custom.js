...
function replaceRuneTooltip(tooltip, queryUnit, requiredHero)
{
    ...
    var runeSkill = ["Q", "W", "E", "R"];
    var runeTier = [1, 2, 3, 4];
    var regex;
    var ability;
    var skill_tree_data;
    runeSkill.forEach(function (skill, skillIndex) {
        runeTier.forEach(function (tier) {
            if (tooltip.indexOf("[" + skill + tier + "]") > -1) {
                regex = new RegExp("\\[" + skill + tier + "\\]", "g");
                if (queryUnit > 0) {
...
function getVoltexSkill(slot){
    var skillName = "";
	if (slot === 1){
        skillName = "DOTA_Tooltip_Ability_voltex_overcharge";
	}else if(slot === 2){
        skillName = "DOTA_Tooltip_Ability_voltex_static";
	}else if(slot === 3){
        skillName = "DOTA_Tooltip_Ability_voltex_azure_leap";
	}else if(slot === 4){
        skillName = "DOTA_Tooltip_Ability_voltex_static_field";
	}
    return skillName;
}
...
function getChernobogSkill(slot){
	...
	if (slot === 1){
        ...
        skillName = "DOTA_Tooltip_Ability_chernobog_4_r";
	}
    return skillName;
}
