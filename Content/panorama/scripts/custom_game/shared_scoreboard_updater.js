...
function uninvisIncomeLabel(){
	// $.GetContextPanel().FindChildTraverse("pvp_secondary_data").RemoveClass('invisible')
	$.GetContextPanel().pvp = true
}
...
function _ScoreboardUpdater_UpdatePlayerPanel( scoreboardConfig, playersContainer, playerId, localPlayerTeamId )
{
	...
	var scoreboardPlayerPanel = playerPanel.FindChild( 'ScoreboardPlayer' )
	playerPanel.SetHasClass( "is_local_player", ( playerId == Game.GetLocalPlayerID() ) );
	// scoreboardPlayerPanel.AddClass("ScoreboardPlayer")
	if (!($.GetContextPanel().premiumArray === undefined)){
		if ($.GetContextPanel().premiumArray.indexOf(playerId) > -1)
		{
	...
	if ( playerInfo )
	{
		...
		_ScoreboardUpdater_SetTextSafe( playerPanel, "PlayerScore", playerInfo.player_kills )
		if ($.GetContextPanel().pvp){
			var hero = Players.GetPlayerHeroEntityIndex( playerId )
			var incomeData = CustomNetTables.GetTableValue( "premium_pass",  "line_war_income_"+hero)
			if (!(incomeData===undefined)){
				_ScoreboardUpdater_SetTextSafe( playerPanel, "PlayerScore", incomeData.income )
			}
		}
		...
		if ( playerPortrait )
		{
			if ( playerInfo.player_selected_hero !== "" )
			{
				...
				playerPortrait.heroIndex = Players.GetPlayerHeroEntityIndex(playerId)
			}
			else
			{
...
{
	$.Msg("CLICKED PORTRAIT")
}
function specialBorder(msg)
{
...
(function () {
GameEvents.Subscribe( "createLineWarIncomeTimer", uninvisIncomeLabel);
})();