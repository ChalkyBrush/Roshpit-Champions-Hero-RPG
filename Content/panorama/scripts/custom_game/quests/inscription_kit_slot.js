...
function OnDragDrop( panelId, draggedPanel )
{
	...
	if (draggedPanel.fromInventory && isValidItem(draggedItem)){
			...
			GameEvents.SendCustomGameEventToServer( "inscription", {event_type: 0, playerID: playerID, heroIndex: heroIndex, itemIndex: draggedItem, kit: $.GetContextPanel().vessel});
			...
			$.GetContextPanel().vesselParent.FindChildTraverse('inscription_input_container').RemoveClass("invisible")

			$.GetContextPanel().vesselParent.FindChildTraverse('final_combine_button_label').text = $.Localize("#inscription_inscribe")
...
function isValidItem(item){
	...
	if (!(itemValues === undefined)){
		if (itemValues.gear_slot == 0 || itemValues.gear_slot == 1 || itemValues.gear_slot == 2 || itemValues.gear_slot == 3 || itemValues.gear_slot == 4 || itemValues.gear_slot == 5){
